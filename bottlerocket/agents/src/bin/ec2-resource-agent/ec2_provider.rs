...
/// Convert the device mappings from the config to the AWS SDK's `BlockDeviceMapping` type.
fn block_device_mappings(
    device_mappings: &Option<Vec<DeviceMapping>>,
) -> Option<Vec<BlockDeviceMapping>> {
    device_mappings.as_ref().map(|mappings| {
        mappings
            .iter()
            .map(|mapping| {
